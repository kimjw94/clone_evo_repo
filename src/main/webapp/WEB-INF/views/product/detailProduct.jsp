		...
		<c:forEach var="p" items="${detailProduct }">
			...
			<div>
				...
				<div>
					<label>
						<select id="category">
						<option value="choice">${p.C_CATEGORY_NAME }</option>
					...
					<label>
						<select id="p_category_code" name="p_category_code" id="p_category_code">
							<option>${p.D_CATEGORY_DETAIL_NAME }</option>
						</select>
					</label>
				</div>
			</div>
			...
			<div>
				<c:forEach var="i" items="${detailInventory}">
					<div class="proTitle">색상 및 사이즈, 수량</div>
					<div id="iAdd">
						<input name="i_product_color1" id="i_product_color1" style="width:66px;" placeholder="색상" value="${i.I_PRODUCT_COLOR }">
						<input name="i_product_size1" id="i_product_size1" style='width:66px; text-transform: uppercase;' placeholder="사이즈" value="${i.I_PRODUCT_SIZE }">
						<input name="i_product_count1" id="i_product_count1" style='width:66px;' type='number' placeholder="수량" value="${i.I_PRODUCT_COUNT }">	
					</div>
				</c:forEach>
			</div>
			<div>
